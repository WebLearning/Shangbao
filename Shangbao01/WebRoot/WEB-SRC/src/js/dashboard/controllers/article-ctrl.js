...
angular.module("Dashboard").controller("articleCtrl", ["$scope","$http", function ($scope,$http) {
    ...
    $scope.recvData=
        ...
        "source":"",
        "keyWords":"",
        "sorts":{"原创":false,"本地":false,"热门":false,"四川":false,"娱乐":false,"时事":false,"体育":false,"财经":false},
        "pictures":[],
        "textContent":""
        };
        ...
        for(p in tempData){
            if(p=="sorts"){
                for(i in $scope.recvData[p]){
                    $scope.recvData[p][i]=tempData[p][i];
                }
            }else{
                $scope.recvData[p]=tempData[p];
            }
        }
    };
